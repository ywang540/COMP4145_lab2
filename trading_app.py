...
import sys
sys.path.insert(0, '/hkbu/COMP4145_lab2')
...
if data is not None and positions is not None:
    ...
    with tab3:
        ...
        if positions.empty:
            ...
            for idx, trade in positions.iterrows():
                ...
                with st.expander(f"{color} Trade #{idx + 1}: {trade['BuyDate'].strftime('%Y-%m-%d')} → {trade['SellDate'].strftime('%Y-%m-%d')} ({trade['ProfitPct']:.2f}%)"):
    ...
    st.error("Unable to load trading data. Please check your internet connection.")
